...
app.use('*', async (c, next) => {
  ...
  if (path.includes('/organizations') || path.includes('/auth/me')) {
    ...
    if (path.includes('/organizations')) {
      console.log('🚨 [DEBUG ORGANIZATIONS] === REQUISIÇÃO POST /organizations DETECTADA ===');
      ...
      try {
        ...
        console.log('🚨 [DEBUG ORGANIZATIONS] Body:', body ? JSON.stringify(body).substring(0, 200) : 'null');
      } catch (e) {
...
app.get("/rendizy-server/make-server-67caf26a/organizations", organizationsRoutes.listOrganizations);
app.get("/rendizy-server/make-server-67caf26a/organizations/slug/:slug", organizationsRoutes.getOrganizationBySlug); // ✅ ANTES de /:id
app.get("/rendizy-server/make-server-67caf26a/organizations/:id/settings/global", organizationsRoutes.getOrganizationSettings); // ✅ ANTES de /:id
app.get("/rendizy-server/make-server-67caf26a/organizations/:id/stats", organizationsRoutes.getOrganizationStats); // ✅ ANTES de /:id
app.get("/rendizy-server/make-server-67caf26a/organizations/:id", organizationsRoutes.getOrganization); // ✅ GENÉRICA por último
app.post("/rendizy-server/make-server-67caf26a/organizations", organizationsRoutes.createOrganization);
app.patch("/rendizy-server/make-server-67caf26a/organizations/:id", organizationsRoutes.updateOrganization);
app.delete("/rendizy-server/make-server-67caf26a/organizations/:id", organizationsRoutes.deleteOrganization);
app.put("/rendizy-server/make-server-67caf26a/organizations/:id/settings/global", organizationsRoutes.updateOrganizationSettings);

// ✅ Novas rotas sem hash (usadas pelo frontend atual)
// ⚠️ IMPORTANTE: Ordem das rotas - rotas específicas ANTES de genéricas
app.get("/rendizy-server/organizations", organizationsRoutes.listOrganizations);
app.get("/rendizy-server/organizations/slug/:slug", organizationsRoutes.getOrganizationBySlug); // ✅ ANTES de /:id
app.get("/rendizy-server/organizations/:id/settings/global", organizationsRoutes.getOrganizationSettings); // ✅ ANTES de /:id
app.get("/rendizy-server/organizations/:id/stats", organizationsRoutes.getOrganizationStats); // ✅ ANTES de /:id
app.get("/rendizy-server/organizations/:id", organizationsRoutes.getOrganization); // ✅ GENÉRICA por último
...
Deno.serve((req) => {
  ...
  console.log('🚨 [DEBUG SERVER] Headers:', Object.fromEntries(req.headers.entries()));
  
  // ✅ DEBUG ESPECÍFICO: Log detalhado para /organizations
  if (pathname.includes('/organizations')) {
    console.log('🚨 [DEBUG ORGANIZATIONS] === REQUISIÇÃO /organizations NO DENO.SERVE ===');
    console.log('🚨 [DEBUG ORGANIZATIONS] Method:', method);
    console.log('🚨 [DEBUG ORGANIZATIONS] Pathname:', pathname);
    console.log('🚨 [DEBUG ORGANIZATIONS] Search:', url.search);
    console.log('🚨 [DEBUG ORGANIZATIONS] Headers completos:', Object.fromEntries(req.headers.entries()));
  }
