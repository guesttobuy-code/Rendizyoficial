...
async function fetchAPI<T>(
  ...
  try {
    ...
    if (!response.ok) {
      // Se o JSON tem success=true, usar mesmo com status HTTP não-OK
      if (json.success === true && json.data) {
        console.warn(`⚠️ [fetchAPI] HTTP ${response.status} mas json.success=true, usando dados mesmo assim`);
        return json;
      }
