...
export default function ContentDescriptionStep({
  ...
  const handleFixedFieldChange = (
    ...
    if (sanitizedValue !== newValue) {
      toast.warning("Emojis não são permitidos", {
        description:
          "Campos fixos não podem conter emojis (política do Airbnb)",
      });
    }
  ...
  const handleAutoTranslateField = async (
    fieldId: string,
    fieldType: "fixed" | "custom"
  ) => {
    const ptText =
      fieldType === "fixed"
  ...
  const fixedFieldsCompleted = FIXED_FIELDS.filter((f) =>
    fixedFields[f.id]?.pt?.trim()
  ).length;

  const customFieldsCompleted = configuredCustomFields.filter((f) =>
    customFieldsValues[f.id]?.pt?.trim()
  ).length;
  ...
  const titleExceedsLimit = title.length > TITLE_MAX_LENGTH;
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="border-2">
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            ...
            <Badge variant={titleExceedsLimit ? "destructive" : "secondary"}>
        ...
        <CardContent className="space-y-2">
          <Input
            ...
            maxLength={TITLE_MAX_LENGTH + 20} // Permitir digitar um pouco mais para mostrar o aviso
            className={
              titleExceedsLimit ? "border-red-500 focus:border-red-500" : ""
            }
          />
          {titleExceedsLimit && (
            <Alert variant="destructive">
              ...
              <AlertDescription>
                <strong>Atenção:</strong> O título ultrapassou{" "}
                {TITLE_MAX_LENGTH} caracteres. No Airbnb, apenas os primeiros{" "}
                {TITLE_MAX_LENGTH} caracteres serão exibidos. Booking e outras
                plataformas não têm essa limitação.
              </AlertDescription>
            </Alert>
          )}
          {title.length > 0 && title.length <= TITLE_MAX_LENGTH && (
            <p className="text-xs text-muted-foreground">
              ✓ Título dentro do limite do Airbnb ({title.length}/
              {TITLE_MAX_LENGTH} caracteres)
            </p>
          )}
        </CardContent>
      </Card>
      ...
      <Alert>
        ...
        <AlertDescription>
          <strong>Política do Airbnb:</strong> Emojis não são permitidos nos
          campos fixos. Você pode usar emojis apenas nos campos personalizados.
        </AlertDescription>
      </Alert>
                ...
                <div className="flex items-center justify-between">
                  ...
                  {autoTranslate && (
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        handleAutoTranslateField(field.id, "fixed")
                      }
                    >
                  ...
                  {(["pt", "en", "es"] as Language[]).map((lang) => (
                    <TabsContent key={lang} value={lang} className="space-y-2">
                      <Textarea
                        ...
                        onChange={(e) =>
                          handleFixedFieldChange(field.id, lang, e.target.value)
                        }
                        rows={4}
                      ...
                      <div className="flex items-center justify-between text-xs text-muted-foreground">
                        ...
                        <span>
                          {fixedFields[field.id]?.[lang]?.length || 0}/
                          {field.maxChars}
                        </span>
                      </div>
                    </TabsContent>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {autoTranslate && (
                          <Button
                            ...
                            size="sm"
                            onClick={() =>
                              handleAutoTranslateField(field.id, "custom")
                            }
                          >
                            <Sparkles className="h-3 w-3 mr-1" />
                            Traduzir
                      ...
                      {(["pt", "en", "es"] as Language[]).map((lang) => (
                        ...
                        >
                          ...
                          <div className="flex items-center justify-between text-xs text-muted-foreground">
                            ...
                            <span>
                              {customFieldsValues[field.id]?.[lang]?.length ||
                                0}{" "}
                              caracteres
                            </span>
                          </div>
      ...
      {configuredCustomFields.length === 0 && (
        <Alert>
          ...
          <AlertDescription>
            ...
            <strong>Configurações → Locais & Anúncios</strong> para adicionar
            campos extras como GPS, Senhas, Instruções, etc.
          </AlertDescription>
        </Alert>
      )}
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-2">
              ...
              <div className="text-sm text-blue-700 space-y-1">
                <div>
                  ✓ {fixedFieldsCompleted} de {FIXED_FIELDS.length} campos fixos
                  preenchidos
                </div>
                <div>
                  ✓ {customFieldsCompleted} de {configuredCustomFields.length}{" "}
