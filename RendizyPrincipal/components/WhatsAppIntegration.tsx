...
export default function WhatsAppIntegration() {
  ...
  const checkWhatsAppStatus = async () => {
    ...
    try {
      ...
      console.log('📊 [WhatsApp] Resposta completa do status:', JSON.stringify(result, null, 2));
      ...
      if (result.success && result.data) {
        ...
        const statusToSet: SessionStatus = isConnected ? 'CONNECTED' : 'DISCONNECTED';
        console.log('📊 [WhatsApp] Status determinado:', statusToSet, 'connected:', isConnected);
        setRealTimeStatus(statusToSet);

        // ✅ CORREÇÃO: Sempre atualizar config com status atual, garantindo persistência
        if (config) {
        ...
        console.warn('⚠️ [WhatsApp] Status check retornou erro:', result.error);
        console.warn('⚠️ [WhatsApp] Detalhes do erro:', {
          success: result.success,
          error: result.error,
          hasData: !!result.data
        });
        
        // ✅ CORREÇÃO: Se erro for "Not found" mas temos data, usar data mesmo assim
        if (result.data && typeof result.data === 'object') {
          const data = result.data as any;
          if (data.connected !== undefined) {
            const isConnected = data.connected === true;
            const statusToSet: SessionStatus = isConnected ? 'CONNECTED' : 'DISCONNECTED';
            console.log('✅ [WhatsApp] Usando dados do result.data mesmo com success=false:', statusToSet);
            setRealTimeStatus(statusToSet);
            
            // Atualizar config se tiver dados
            if (config) {
              setConfig({
                ...config,
                whatsapp: {
                  ...
                  connected: isConnected,
                  connection_status: isConnected ? 'connected' : 'disconnected',
                  phone_number: data.phone_number || config.whatsapp?.phone_number
                }
              });
            }
            return; // Sair aqui se conseguimos processar os dados
          }
        }
      ...
      if (error instanceof Error && (error.message.includes('Failed to fetch') || error.message.includes('Network'))) {
        ...
        // Não mudar status se for erro de rede
      } else {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        if (result.data.whatsapp?.connected) {
          ...
          console.log('✅ [WhatsApp] Status inicial carregado: CONNECTED');
        } else if (result.data.whatsapp?.connection_status === 'connecting') {
          setRealTimeStatus('CONNECTING');
          console.log('⏳ [WhatsApp] Status inicial carregado: CONNECTING');
        } else {
          setRealTimeStatus('DISCONNECTED');
    ...
    try {
      ...
      try {
        ...
        if (result.success) {
          ...
          // Verificar status e atualizar na tela
                ...
                <div className="flex items-center justify-between">
                    ...
                    <p className="text-2xl mt-1">
                      {(() => {
                        ...
                        if (realTimeStatus === 'ERROR' && config?.whatsapp?.connected) {
                          // Se deu erro na verificação mas config diz conectado, usar CONNECTED
                          statusToShow = 'CONNECTED';
                          console.log('⚠️ [WhatsApp] Status check deu erro, mas config salva diz conectado - usando CONNECTED');
                        } else if (realTimeStatus !== null) {
                          // Usar status em tempo real se disponível
                          statusToShow = realTimeStatus;
                        } else {
                  ...
                  <div className={`w-12 h-12 rounded-full ${
                    (() => {
                      ...
                      } else if (realTimeStatus === 'CONNECTED' || (!realTimeStatus && config?.whatsapp?.connected)) {
                        return 'bg-green-500/10';
