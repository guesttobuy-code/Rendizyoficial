 ...
 */
function validateContentLocation(data: any): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
 ...
 */
function validateContentDescription(data: any): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
...
app.post('/create', async (c) => {
  try {
    ...
    const tenantKey = `tenant:${tenantId}:properties`;
    ...
    console.log(`[Property Wizard] Propriedade criada: ${propertyId} (tenant: ${tenantId})`);
...
app.put('/:id/step/:stepId', async (c) => {
  try {
    ...
    const property = await kv.get(`property:${propertyId}`) as PropertyWizardData;
    ...
    const requiredSteps = ['content-type', 'content-location', 'content-description'];
    ...
    console.log(`[Property Wizard] Step atualizado: ${propertyId} -> ${stepId}`);
...
app.get('/tenant/:tenantId', async (c) => {
  try {
    ...
    const propertyIds = await kv.get(`tenant:${tenantId}:properties`) || [];
    
    const properties = await Promise.all(
      propertyIds.map(async (id: string) => {
        return await kv.get(`property:${id}`);
      })
    );
...
app.delete('/:id', async (c) => {
  try {
    ...
    const tenantKey = `tenant:${property.tenantId}:properties`;
    const existingProperties = await kv.get(tenantKey) || [];
    const updatedProperties = existingProperties.filter((id: string) => id !== propertyId);
